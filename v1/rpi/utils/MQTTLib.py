...
class MQTT:
    ...
    def __init__(self, mqttClientID, onConnectMethod=None, onMessageMethod=None):
        ...
        self.mqttChannel = {
            ...
            "publish": "channel/" + str(mqttClientID) + "/broadcast",
        ...
        self.client.publish(topic=self.mqttChannel["status"], payload='{"status": "on", "timestamp": ' + str(datetime.datetime.now()) + '}')
...
if (( config.property["mock"]["isMock"] ) and (__file__ == "MQTTLib.py")):
    ...
    def customOnMessage(*argsM):
        ...
        print(argsM[0]["message"].payload.decode("utf-8"))
